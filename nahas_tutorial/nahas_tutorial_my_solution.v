...
The command "refine (A_implies_B _)." lets us create the proof of B
...
It should be easy for you to prove that "not" and "notb" are
...
Theorem thm_exists_basics_ : (ex basic_predicate).
Proof.
  pose (witness := true).
  exists witness.
  exact I.
...
Proof.
  ...
  intro antec0.
  intro antec1.
  destruct antec1 as [witn p_of_witn].
  unfold not in antec0.
  pose (antec1 := antec0 witn).
  pose (proof_of_false := antec1 p_of_witn).
  exact proof_of_false.
Qed.
...
Proof.
  ...
  intro Px.
  unfold not in antec0.
  pose (exist := ex_intro P x Px).
  pose (proof_of_false := antec0 exist).
  exact proof_of_false.
Qed.
...
lets Coq infer the type "A" instead of forcing you to type it.
...
Proof.
  ...
  elim l0.
    ...
    intro A1.
    intro l1.
    intro l1_app_nil_is_l1.
    simpl.
    rewrite l1_app_nil_is_l1.
    exact (eq_refl (A1 :: l1)).
Qed.
...
Proof.
  ...
  case y.
    simpl.
    intro nil_is_a_cons_nil.
    discriminate nil_is_a_cons_nil.
    
    intros a0 l0.
    simpl.
    intro nil_is_double_cons.
    discriminate nil_is_double_cons.

    intros a1 l1.
    simpl.
    intro nil_is_cons_app_cons.
    discriminate nil_is_cons_app_cons.
Qed.
